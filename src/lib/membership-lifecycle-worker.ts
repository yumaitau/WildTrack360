...
import { Prisma, type MembershipNotificationKind } from '@prisma/client';
...
export async function runMembershipLifecycle(
  ...
  // Send one lifecycle email, guarded by the unique notification row.
  async function notifyOnce(
    ...
    if (!sent) {
      await prisma.membershipNotification.deleteMany({
        where: { membershipId: membership.id, kind },
      });
      return;
    }
