...
export async function PATCH(request: Request) {
  ...
  try {
    // Verify the item belongs to this org before updating
    const existing = await getModel(type).findFirst({
      where: { id: body.id, clerkOrganizationId: orgId },
    })
...
export async function DELETE(request: Request) {
  ...
  try {
    // Verify the item belongs to this org before deleting
    const existing = await getModel(type).findFirst({
