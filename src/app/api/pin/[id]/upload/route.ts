...
import { uploadPinPhotoContract } from '../../openapi';
...
export const POST = route(uploadPinPhotoContract, async ({ query, params, request }) => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (current.photoUrls.length >= MAX_PHOTOS_PER_SESSION) throw new Error('PHOTO_LIMIT');
