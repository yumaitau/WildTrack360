...
import { authenticateReportUser, canSaveReports, ReportAccessError } from '@/lib/custom-query/access';
...
import { listQueriesContract, createQueryContract } from './openapi';
...
export const GET = route(listQueriesContract, async () => {
  try {
    const { orgId } = await authenticateReportUser();
    const queries = await prisma.savedReportQuery.findMany({ where: { orgId }, orderBy: { createdAt: 'desc' } });
    return { data: queries };
  } catch (error) {
...
export const POST = route(createQueryContract, async ({ body }) => {
  try {
    const { userId, orgId, role } = await authenticateReportUser();
    if (!canSaveReports(role)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    ...
    const created = await prisma.savedReportQuery.create({
      data: { orgId, createdByUserId: userId, name, query: ast.raw, visualization: visualization ?? ast.visualization, showOnDashboard: showOnDashboard ?? false },
    });
    logAudit({ userId, orgId, action: 'CREATE', entity: 'SavedReportQuery', entityId: created.id, metadata: { name } });
