...
const {
  ...
  mockGetPortalMember,
  mockGateFeature,
  mockListPublishedNews,
} = vi.hoisted(() => ({
  mockAuth: vi.fn(),
  mockGetPortalMember: vi.fn(),
  mockGateFeature: vi.fn(),
  mockListPublishedNews: vi.fn(),
}));
...
vi.mock('@/lib/news', () => ({ listPublishedNews: mockListPublishedNews }));
...
function getReq() {
  return new Request('http://t.localhost/api/portal/news', { method: 'GET' });
}

describe('GET /api/portal/news', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockAuth.mockResolvedValue({ userId: 'user-1' });
    mockGetPortalMember.mockResolvedValue(baseSession);
    mockGateFeature.mockResolvedValue(null);
    mockListPublishedNews.mockResolvedValue([basePost]);
  });
  ...
  it('returns 200 with schema-valid posts (Date as ISO string)', async () => {
    const res = await GET(getReq());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body)).toBe(true);
    expect(body[0].id).toBe('post-1');
    expect(body[0].publishedAt).toBe('2026-01-01T00:00:00.000Z');
    expect(body[0].authorName).toBeNull();
  });
});
