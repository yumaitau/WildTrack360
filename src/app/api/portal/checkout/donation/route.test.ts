...
const {
  ...
  mockGetPortalMember,
  mockGateFeature,
  mockCreateDonationPayment,
} = vi.hoisted(() => ({
  mockAuth: vi.fn(),
  mockGetPortalMember: vi.fn(),
  mockGateFeature: vi.fn(),
  mockCreateDonationPayment: vi.fn(),
}));
...
vi.mock('@/lib/square/checkout', () => ({ createDonationPayment: mockCreateDonationPayment }));
...
function postReq(body: unknown) {
  return new Request('http://t.localhost/api/portal/checkout/donation', {
    method: 'POST',
...
describe('POST /api/portal/checkout/donation', () => {
  ...
  it('returns 404 when feature is gated', async () => {
    ...
    const res = await POST(postReq({ amountCents: 1000, sourceId: 'nonce-1' }));
    expect(res.status).toBe(404);
  });

  it('returns 200 with schema-valid checkout result', async () => {
    const res = await POST(postReq({ amountCents: 1000, sourceId: 'nonce-1' }));
