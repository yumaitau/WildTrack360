...
export const GET = route(eofyReportContract, async ({ query }) => {
  ...
  const fyEndYear = query.fy ? parseFinancialYearEndYear(query.fy) : financialYearEndYear(new Date());
  if (!fyEndYear) return NextResponse.json({ error: 'Invalid financial year' }, { status: 400 });

  const donors = await aggregateDonorsForFy(orgId, fyEndYear);
  return { data: { fyEndYear, donors } };
});
